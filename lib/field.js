...
class Field {
    ...
    setFieldsValue(body, callback) {
        ...
        async.forEachOf(
            self.fields,
            (field, fieldName, cb) => {
                if(body.hasOwnProperty(fieldName) && typeof body[fieldName] === 'string') {
                    field.value = body[fieldName].trim() || ''
                } else {
                    field.value = body[fieldName] || ''
                }
                return cb()
            },
            () => {
    ...
    checkType(field, cb) {
        ...
        if('string' === field.type || 'text' === field.type || 'mixed' === field.type) return cb(null)
