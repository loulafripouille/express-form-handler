...
var Form = function() {
    ...
    this.handleRequest = function()
    {
        ...
        return function(req, res, next) {
            ...
            self.Field.Constraint = new Constraint(self.Field.ErrorHandler, self.Field.fields);

            //Do the form assertion on each field
            async.forEachOf(
                self.Field.fields,
                function(field, fieldName, cb) {
                    field.value = req.body[fieldName].trim() || '';
                    self.Field.checkField(field, self.locale, function(err) {
                        if (err) {
                            return cb(err);
                        }
                        //Populate the form field value in the req.form object
