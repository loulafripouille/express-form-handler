...
Equalstorule.prototype.check = function (field, fields) {
  if (!fields.hasOwnProperty(this.target)) {
    throw new Error('the given target passed to equalsto rule doesn\'t exist')
  }
  if (!assert.deepEqual(field.value, fields[this.target].value)) {
    this.error = `The field ${field.label} must match the field ${fields[this.target].label} value`
    return false
  }
